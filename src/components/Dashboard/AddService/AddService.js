...
const AddService = ({ editService, restrictPermission, setEditService }) => {
  ...
  const onSubmit = async (data) => {
    ...
    if (!editService || (editService && data.image[0])) {
      ...
      imageData.set("key", "955fa34c0f50537acbd9b2af464e5ae6");
    ...
    if (editService) {
      ...
      axios
        .patch(
          `https://pixartisan.herokuapp.com/update/${editService._id}`,
          serviceInfo
        )
        .then((res) => {
          toast.dismiss(loading);
    ...
    axios
      .post("https://pixartisan.herokuapp.com/addService", serviceInfo)
      .then((res) => {
        toast.dismiss(loading);
        if (res.data) {
          return swal(
